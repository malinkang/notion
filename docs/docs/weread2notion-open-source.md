...
| `NOTION_DATABASE_ID` | Notion 数据库 ID，用于确定同步到哪个数据库 | 必填 |
...
| `NOTION_DATABASE_ID` | Notion 模板中的数据库 ID |
...
## 获取 Notion Token 和数据库 ID

你可以继续使用原来的 Notion 授权方式获取 `NOTION_TOKEN`，然后从模板数据库链接中复制 `NOTION_DATABASE_ID`：
...
5. 打开 Notion 模板中的书籍数据库，复制数据库链接中的 ID，保存为 `NOTION_DATABASE_ID`。
6. 回到 GitHub Secrets 中保存这几个值。

::: tip
新版脚本使用 Notion `2026-03-11` 版本和 data source API。你只需要配置 `NOTION_DATABASE_ID`，脚本会自动解析并使用对应的 data source。
:::
...
export NOTION_DATABASE_ID="你的 Notion 数据库 ID"
weread2notion sync
```
